...
def get_job(playlist_id: str | uuid.UUID):
    return scheduler.get_job(job_id=playlist_id)


def create_job(func: Callable, kwargs: dict, cron_expression: str):
    playlist_id = str(cast(Playlist, kwargs.get("playlist")).id)
    job = get_job(playlist_id=playlist_id)
...
def update_job(func: Callable, kwargs: dict, cron_expression: str):
    playlist_id = str(cast(Playlist, kwargs.get("playlist")).id)
    job = get_job(playlist_id=playlist_id)
...
def delete_job(playlist_id: str | uuid.UUID):
    job = get_job(playlist_id=playlist_id)
